...
class StockReport(models.TransientModel):
    ...
    checking = fields.Many2many('employees.checking', string='Warehouse', required=True)
    ...
    def export_xls(self):
        ...
        return {
            ...
            'data': {'model': 'employee.history',
                     ...
                     'output_format': 'xlsx',
                     'report_name': 'Lịch sử chấm công',
                     }
        }

    def get_warehouse(self, data):
        wh = data.checking.mapped('id')
        obj = self.env['employees.checking'].search([('id', 'in', wh)])
        l1 = []
        l2 = []
        for j in obj:
            l1.append(j.employee_id)
            l2.append(j.id)
        return l1, l2
    ...
    def get_lines(self, data, warehouse):
        ...
        if categ_id:
            categ_products = self.env['employees.checking'].search([('id', 'in', categ_id)])
